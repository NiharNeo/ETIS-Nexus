...
import { Calendar, Clock, MapPin, Users, ExternalLink, Tag, Ticket, CheckCircle2, X, ShieldCheck, Send } from 'lucide-react';
...
export function EventDetailModal({ event, club, open, onClose }: EventDetailModalProps) {
  ...
  const { registerForEvent, approveEvent, rejectEvent, submitForApproval, getRegistrations } = useData();
  const { user } = useAuth();
              ...
              {user?.role === 'super_admin' ? (
                <div className="flex flex-col gap-6 pt-6">
                  {event.status === 'pending' || event.status === 'rejected' ? (
                    ...
                    <div className="text-center p-6 rounded-3xl bg-emerald-500/5 border border-emerald-500/20">
